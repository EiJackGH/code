...
## 2025-03-24 - Screen Reader Game Feedback
**Learning:** Static instructions inside `aria-live` regions force screen readers to unnecessarily repeat the static text every time the dynamic content updates.
**Action:** Extract static text into a separate sibling element or parent container and only apply `aria-live` to the specific node that dynamically updates. Always ensure visible keyboard instructions are provided for custom HTML5 game interfaces.
