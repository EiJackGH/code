...
## 2024-05-24 - Explicit Instructional Text for Custom Key Bindings
**Learning:** When building interactive HTML5 widgets or games with custom keyboard event bindings, users do not natively know the required inputs, leading to poor UX if instructions are missing.
**Action:** Always provide explicit, visible instructional text for custom keyboard interactions and use CSS `pointer-events: none` on overlays to ensure they do not block user interactions.
