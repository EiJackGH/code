...
## 2024-05-10 - Screen Reader Accessibility for Dynamic Scores
**Learning:** Dynamic, JavaScript-updated scores or counters are entirely silent to screen readers by default. This makes game progression unobservable for visually impaired users.
**Action:** Always add `aria-live="polite"` and `aria-atomic="true"` to dynamic score or status elements to ensure real-time screen reader announcements without overwhelming the user.
