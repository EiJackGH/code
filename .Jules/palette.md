...
## 2024-05-23 - Interactive Game UI Controls & Screen Readers
**Learning:** Interactive HTML5 games often lack visible control hints, frustrating users who must guess keyboard bindings. Additionally, dynamically updated UI elements like score counters are silent to screen readers by default.
**Action:** Always add explicit, visual instructions for custom key bindings (like "Press Space to jump") and use `aria-live="polite"` and `aria-atomic="true"` on elements with dynamic textual updates (like game scores) to ensure accessibility.
