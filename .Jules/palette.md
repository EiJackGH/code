...
## 2025-05-18 - HTML5 Game Accessibility and Keyboard Controls
**Learning:** For interactive web widgets or HTML5 games that use keyboard events (like "Space" or "ArrowUp" to jump), screen readers do not natively announce dynamic visual changes (like a score counter). Also, users will not know what keys to press unless explicitly instructed.
**Action:** When building interactive HTML5 widgets or games with custom keyboard event bindings, always provide explicit, visible instructional text so users know the required inputs, and use `aria-live="polite"` on dynamic status updates like scores.
