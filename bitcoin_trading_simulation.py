...
def simulate_trading(signals, initial_cash=10000):
    ...
    for i, row in signals.iterrows():
        ...
        if row['positions'] == 2.0:
            ...
            action_msg = f"{Colors.OKGREEN}🟢 BUY {btc_to_buy:.4f} BTC at ${row['price']:.2f}{Colors.ENDC}"
        ...
        # Sell signal
            ...
            if portfolio.loc[i, 'btc'] > 0:
                ...
                action_msg = f"{Colors.FAIL}🔴 SELL {portfolio.loc[i, 'btc']:.4f} BTC at ${row['price']:.2f}{Colors.ENDC}"
                portfolio.loc[i, 'btc'] = 0

        portfolio.loc[i, 'total_value'] = portfolio.loc[i, 'cash'] + portfolio.loc[i, 'btc'] * row['price']

        day_info = f"Day {i}: Portfolio: ${portfolio.loc[i, 'total_value']:.2f} | Cash: ${portfolio.loc[i, 'cash']:.2f} | BTC: {portfolio.loc[i, 'btc']:.4f}"
        if action_msg:
            print(f"{day_info} | {action_msg}")
        else:
            print(day_info)
...
if __name__ == "__main__":
    ...
    print(f"Final Portfolio Value:        {Colors.BOLD}${final_value:.2f}{Colors.ENDC}")

    if profit >= 0:
        profit_color = Colors.OKGREEN
        profit_icon = "📈"
    else:
        profit_color = Colors.FAIL
        profit_icon = "📉"

    print(f"Profit/Loss:                  {profit_color}{profit_icon} ${profit:.2f}{Colors.ENDC}")
