...
def simulate_trading(signals, initial_cash=10000):
    ...
    print(f"{Colors.PURPLE}------ Daily Trading Ledger ------{Colors.RESET}")
...
if __name__ == "__main__":
    ...
    print(f"\n{Colors.PURPLE}------ Final Portfolio Performance ------{Colors.RESET}")
    print(f"Initial Cash: ${initial_cash:.2f}")
    print(f"Final Portfolio Value: ${final_value:.2f}")

    profit_color = Colors.GREEN if profit >= 0 else Colors.RED
    profit_icon = "📈" if profit >= 0 else "📉"
    print(f"Profit/Loss: {profit_color}{profit_icon} ${profit:.2f}{Colors.RESET}")
