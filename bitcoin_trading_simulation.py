...
def simulate_trading(signals, initial_cash=10000):
    ...
    # UX Fix: Explicitly cast to float to prevent FutureWarning
    ...
    for i, row in signals.iterrows():
        ...
        if row['positions'] == 2.0:
            ...
            print(f"Day {i}: {Colors.GREEN}🟢 Buy{Colors.ENDC} {btc_to_buy:.4f} BTC at ${row['price']:.2f}")
        ...
        # Sell signal
            ...
            if portfolio.loc[i, 'btc'] > 0:
                ...
                print(f"Day {i}: {Colors.RED}🔴 Sell{Colors.ENDC} {portfolio.loc[i, 'btc']:.4f} BTC at ${row['price']:.2f}")
                portfolio.loc[i, 'btc'] = 0

        portfolio.loc[i, 'total_value'] = portfolio.loc[i, 'cash'] + portfolio.loc[i, 'btc'] * row['price']

        # UX Improvement: Only print daily status if something interesting happened or periodically?
        # For now, keeping it verbose but formatted.
        # Maybe grey out the boring days? Or just keep plain.
...
if __name__ == "__main__":
    ...
    print(f"Profit/Loss: {profit_color}${profit:.2f}{Colors.ENDC}")
