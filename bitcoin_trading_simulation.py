...
def simulate_trading(signals, initial_cash=10000, quiet=False):
    ...
    is_interactive_quiet = quiet and sys.stdout.isatty()
    ...
    for idx, (i, row) in enumerate(signals.iterrows()):
        ...
        elif is_interactive_quiet:
            progress = int(((idx + 1) / total_days) * 100)
            bar_length = 20
            filled_length = int(bar_length * (idx + 1) // total_days)
            bar = '█' * filled_length + '-' * (bar_length - filled_length)
            sys.stdout.write(f"\r{Colors.CYAN}Simulating: [{bar}] {progress}%{Colors.ENDC}")
            sys.stdout.flush()

    if is_interactive_quiet:
